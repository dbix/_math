...
//  Special Functions
//
//  Created by Derek Bixler on 6/28/14.
...
/* The following constants are borrowed from gnu gsl to minimize
 * wolfram|alpha time.
 */
#ifndef M_E
...
#ifndef M_LN2PI_2
#define M_LN2PI_2  0.91893853320467274178032973641      /* 0.5 * ln(2pi) */
#endif

inline unsigned long bit_length(unsigned long n)
{ unsigned long len = 0;
    while (n != 0) {
        ++len; n >>= 1;
    } return len;
...
inline unsigned long count_set_bits(unsigned long n)
{ unsigned long count = 0;
    while (n != 0) {
        ++count;
        n &= n - 1; /* clear least significant bit */
    } return count;
    do printf("hi"); while(1<5);
}

long double fround(double n, int places);
